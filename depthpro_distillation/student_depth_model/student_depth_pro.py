...
from .student_decoder import MultiresConvDecoder
from .student_encoder import DepthProEncoder
#from .network.fov import FOVNetwork
from .student_vit_factory import VIT_CONFIG_DICT, ViTPreset, create_vit
...
DEFAULT_MONODEPTH_CONFIG_DICT = DepthProConfig(
    patch_encoder_preset="dinov2b8_96",
    image_encoder_preset="dinov2b8_96",
...
def create_model_and_transforms(
    ...
    #device: torch.device = torch.device("cuda", torch.cuda.current_device()),
    device: torch.device = torch.device("cpu"),
    precision: torch.dtype = torch.float32,
) -> Tuple[StudentDepthPro, Compose]:
...
class StudentDepthPro(nn.Module):
    """DepthPro network."""
