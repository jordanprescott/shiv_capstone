...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        # Tracking mask
        if globals.is_guiding and globals.current_target_to_guide is not None and is_key_in_dict(globals.current_target_to_guide, globals.objects_data):
            obj_data = globals.objects_data[globals.current_target_to_guide]
            
            target_mask_vis = obj_data.get('mask_vis', None)
            target_class_name = obj_data.get('class', 'unknown')
            target_depth = obj_data.get('depth', 0)
            target_x_angle = obj_data.get('x_angle', 0)
            target_y_angle = obj_data.get('y_angle', 0)

            if target_mask_vis is not None:
                colored_mask = np.zeros_like(raw_frame)
                colored_mask[target_mask_vis > 0] = WHITE  # Safe now
